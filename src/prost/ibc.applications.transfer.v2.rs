...
pub mod query_v2_server {
    ...
    #[async_trait]
    pub trait QueryV2: std::marker::Send + std::marker::Sync + 'static {
        /// Denoms queries all denominations
        async fn denoms(
            &self,
    ...
    impl<T> Clone for QueryV2Server<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "ibc.applications.transfer.v2.QueryV2";
    impl<T> tonic::server::NamedService for QueryV2Server<T> {
